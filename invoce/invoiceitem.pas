...
uses
  Classes, SysUtils, xmlobject, AdditionalInfo, AbstractSerializationObjects;
...
begin
  RegisterProperty('TotalWithVatExcluded', 'СтТовБезНДСВсего', [], 'Всего к оплате, Стоимость товаров (работ, услуг), имущественных прав без налога - всего (строка "Всего к оплате"/графа 5 счета-фактуры)', 1, 19);
  RegisterProperty('Total', 'СтТовУчНалВсего', [], 'Всего к оплате, Стоимость товаров (работ, услуг), имущественных прав с налогом - всего (строка "Всего к оплате"/графа 9 счета-фактуры)', 1, 19);
  RegisterProperty('TotalDef', 'ДефСтТовУчНалВсего', [], 'Всего к оплате, Стоимость товаров (работ, услуг), имущественных прав с налогом - всего (строка "Всего к оплате"/графа 9 счета-фактуры) при отсутствии показателя', 1, 1);
  RegisterProperty('Vat', 'СумНалВсего', [xsaRequared], 'Всего к оплате, Сумма налога, предъявляемая покупателю (строка "Всего к оплате"/графа 8 счета-фактуры)', -1, -1);
  RegisterProperty('TotalNet', 'КолНеттоВс', [xsaSimpleObject], 'Количество (масса нетто/количество) - всего по документу', 1, 26, 'НеттоВс');
end;
...
begin
  RegisterProperty('PackagingIdentificationNumber', 'ИдентТрансУпак', [], 'Уникальный идентификатор транспортной упаковки', 1, 255);
  RegisterProperty('CheckMark', 'КИЗ', [xsaSimpleObject], 'Контрольный идентификационный знак', -1, -1);
  RegisterProperty('SecondaryPackagingIdentificationNumber', 'НомУпак', [xsaSimpleObject], 'Уникальный идентификатор вторичной (потребительской)/третичной (заводской, транспортной) упаковки', -1, -1);
end;
...
begin
  RegisterProperty('BatchNumber', 'НомТовПрослеж', [xsaRequared], 'Регистрационный номер партии товаров', 1, 29);
  RegisterProperty('UnitCode', 'ЕдИзмПрослеж', [xsaRequared], 'Единица количественного учета товара, используемая в целях осуществления прослеживаемости', 3, 4);
  RegisterProperty('UnitName', 'НаимЕдИзмПрослеж', [xsaRequared], 'Наименование единицы количественного учета товара, используемой в целях осуществления прослеживаемости', 1, 255);
  RegisterProperty('Quantity', 'КолВЕдПрослеж', [xsaRequared], 'Количество товара в единицах измерения прослеживаемого товара', 1, 26);
  RegisterProperty('AdditionalInfo', 'ДопПрослеж', [], 'Дополнительный показатель для идентификации товаров, подлежащих прослеживаемости', 1, 255);
end;
...
begin
  RegisterProperty('ItemMark', 'ПрТовРаб', [], 'Признак Товар/Работа/Услуга/Право/Иное', 1, 1);
  RegisterProperty('AdditionalProperty', 'ДопПризн', [], 'Дополнительная информация о признаке', 1, 4);
  RegisterProperty('UnitName', 'НаимЕдИзм', [], 'Наименование единицы измерения (условное обозначение национальное, графа 2а счета-фактуры)', 1, 255);
  RegisterProperty('CountryNameShort', 'КрНаимСтрПр', [], 'Краткое наименование страны происхождения товара (графа 10а счета-фактуры)/страна регистрации производителя товара', 1, 255);
  RegisterProperty('ItemToRelease', 'НадлОтп', [], 'Заказанное количество (количество надлежит отпустить)', 1, 26);
  RegisterProperty('ItemCharacteristic', 'ХарактерТов', [], 'Характеристика/описание товара (в том числе графа 1 счета-фактуры)', 1, 1000);
  RegisterProperty('ItemSort', 'СортТов', [], 'Сорт товара', 1, 10);
  RegisterProperty('ItemArticleNumber', 'АртикулТов', [], 'Артикул товара (в том числе графа 1 счета-фактуры)', 1, 50);
  RegisterProperty('ItemVendorCode', 'КодТов', [], 'Код товара (в том числе графа 1 счета-фактуры)', 1, 100);
  RegisterProperty('ItemCatalogCode', 'КодКат', [], 'Код каталога', 27, 27);
  RegisterProperty('ItemTypeCode', 'КодВидТов', [], 'Код вида товара', 10, 10);
  RegisterProperty('TraceabilityInformation', 'СведПрослеж', [], 'Сведения о товаре, подлежащем прослеживаемости', -1, -1);
  RegisterProperty('ProductIdentificationNumber', 'НомСредИдентТов', [], 'Номер средств идентификации товаров', -1, -1);
end;
...
begin
  RegisterProperty('CountryCode', 'КодПроисх', [], 'Цифровой код страны происхождения товара (Графа 10 счета-фактуры)', 3, 3);
  RegisterProperty('CountryCodeDef', 'ДефКодПроисх', [], 'Цифровой код страны происхождения товара (Графа 10 счета-фактуры; для документа с Функция=СЧФ, выставляемом при получении оплаты, частичной оплаты в счет предстоящих поставок товаров (выполнения работ, оказания услуг), передачи имущественных прав', 1, 1);
  RegisterProperty('DeclarationNumber', 'НомерТД', [], 'Регистрационный номер таможенной декларации (номер декларации на товары; графа 11 счета-фактуры)', 1, 29);
end;
...
begin
  RegisterProperty('VatValue', 'СумНал', [xsaRequared, xsaSimpleObject], 'Значение', 1, 19);
  RegisterProperty('WithoutVat', 'БезНДС', [xsaRequared, xsaSimpleObject], 'При определении налоговой базы налоговыми агентами - покупателями (получателями) товаров, перечисленных в пункте 8 статьи 161 НК РФ, продавцами может указываться 0 (ноль; визуализируется как прочерк), если иное не предусмотрено правилами заполнения счета-фактуры, применяемого при расчетах по налогу на добавленную стоимость, утвержденными Постановлением N 1137', 1, 18);
  RegisterProperty('VatValueDef', 'ДефНДС', [xsaRequared, xsaSimpleObject], 'Знак прочерка', 1, 1);
end;
...
begin
  RegisterProperty('Sum', 'СумАкциз', [xsaSimpleObject], 'Сумма акциза', 1, 19);
  RegisterProperty('Without', 'БезАкциз', [xsaSimpleObject], 'Без акциза', 10, 10);
end;
...
begin
  RegisterProperty('LineNumber', 'НомСтр', [xsaRequared], 'Номер строки таблицы', 0, 999999);
  RegisterProperty('Product', 'НаимТов', [], 'Наименование товара (описание выполненных работ, оказанных услуг), имущественных прав (в том числе графа 1 счета-фактуры)', 1, 1000);
  RegisterProperty('UnitCode', 'ОКЕИ_Тов', [xsaRequared], 'Код единицы измерения (графа 2 счета-фактуры)', 3, 4);
  RegisterProperty('UnitCodeDef', 'ДефОКЕИ_Тов', [], 'Код единицы измерения (графа 2 счета-фактуры при составлении документа с Функция=СЧФ или Функция=СЧФДОП при отсутствии данных и для документа с Функция=СЧФ, выставляемом при получении оплаты, частичной оплаты в счет предстоящих поставок товаров (выполнения работ, оказания услуг), передачи имущественных прав)', 1, 1);
  RegisterProperty('Quantity', 'КолТов', [], 'Количество (объем) (графа 3 счета-фактуры)', 1, 26);
  RegisterProperty('Price', 'ЦенаТов', [], 'Цена (тариф) за единицу измерения (графа 4 счета-фактуры)', 1, 26);
  RegisterProperty('SubtotalWithVatExcluded', 'СтТовБезНДС', [], 'Стоимость товаров (работ, услуг), имущественных прав без налога - всего (графа 5 счета-фактуры)', -1, -1);
  RegisterProperty('TaxRate', 'НалСт', [xsaRequared], 'Налоговая ставка (графа 7 счета-фактуры)', 1, 35);
  RegisterProperty('Subtotal', 'СтТовУчНал', [], 'Стоимость товаров (работ, услуг), имущественных прав с налогом - всего (графа 9 счета-фактуры)', 1, 19);
  RegisterProperty('SubtotalDef', 'ДефСтТовУчНал', [], 'Стоимость товаров (работ, услуг), имущественных прав с налогом - всего (графа 9 счета-фактуры) при составлении документа с Функция=СЧФ или Функция=СЧФДОП при отсутствии показателя', 1, 1);
  RegisterProperty('Excise', 'Акциз', [xsaRequared], 'В том числе сумма акциза (графа 6 счета-фактуры)', -1, -1);
  RegisterProperty('Vat', 'СумНал', [xsaRequared], 'Сумма налога, предъявляемая покупателю (графа 8 счета-фактуры)', -1, -1);
  RegisterProperty('CustomsDeclaration', 'СвТД', [], 'Сведения о таможенной декларации', -1, -1);
  RegisterProperty('InvoiceItemAdditional', 'ДопСведТов', [], 'Дополнительные сведения об отгруженных товарах (выполненных работах, оказанных услугах), переданных имущественных правах', -1, -1);
  RegisterProperty('AdditionalInfo', 'ИнфПолФХЖ2', [], 'Информационное поле факта хозяйственной жизни 2', -1, -1);
end;
...
begin
  RegisterProperty('InvoiceItemList', 'СведТов', [xsaRequared], 'Сведения об отгруженных товарах (о выполненных работах, оказанных услугах), переданных имущественных правах', -1, -1);
  RegisterProperty('TotalForPay', 'ВсегоОпл', [xsaRequared], 'Реквизиты строки "Всего к оплате"', -1, -1);
end;
