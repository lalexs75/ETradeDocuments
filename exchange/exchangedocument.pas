...
begin
  RegisterProperty('FileName', 'ИдФайлИнфПр', [xsaRequared], 'Идентификатор файла обмена информации продавца', 1, 255);
  RegisterProperty('DateCreate', 'ДатаФайлИнфПр', [xsaRequared], 'Дата формирования файла обмена информации продавца', 10, 10);
  RegisterProperty('TimeCreate', 'ВремФайлИнфПр', [xsaRequared], 'Время формирования файла обмена информации продавца', 8, 8);
  RegisterProperty('Sign', 'ЭП', [xsaRequared, xsaSimpleObject], 'Электронная подпись файла обмена информации продавца', -1, -1);
end;
...
begin
  ...
  RegisterProperty('DateCreate', 'ДатаИнфПок', [xsaRequared], 'Дата формирования файла обмена информации покупателя', 10, 10);
  RegisterProperty('TimeCreate', 'ВремИнфПок', [xsaRequared], 'Время формирования файла обмена информации покупателя', 8, 8);
  ...
  RegisterProperty('ExchangeFileIdentificatorSeller', 'ИдИнфПрод', [xsaRequared], 'Идентификация файла обмена счета-фактуры (информации продавца) или файла обмена информации продавца', -1, -1);
  RegisterProperty('ExchangeInformation', 'СодФХЖ4', [xsaRequared], 'Содержание факта хозяйственной жизни 4 - сведения о принятии товаров (результатов выполненных работ), имущественных прав (о подтверждении факта оказания услуг)', -1, -1);
  RegisterProperty('TreasuryInformation', 'ИнфПокГосЗакКазн', [], 'Информация покупателя об обстоятельствах закупок для государственных и муниципальных нужд (для учета Федеральным казначейством денежных обязательств)', -1, -1);
  RegisterProperty('ExchangeSigner', 'Подписант', [xsaRequared], 'Сведения о лице, подписывающем файл обмена информации покупателя в электронной форме', -1, -1);
end;
